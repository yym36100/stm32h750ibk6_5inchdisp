...
{ 
	...
	GPIO_Set(GPIOF,PIN10,GPIO_MODE_OUT,GPIO_OTYPE_PP,GPIO_SPEED_HIGH,GPIO_PUPD_PU);	//PF10推挽输出 
 	W25QXX_CS(1);					//SPI FLASH不选中
	SPI2_Init();		   			//初始化SPI
	SPI2_SetSpeed(SPI_SPEED_4);		//设置为50M时钟,高速模式 
...
{ 
 	u16 i;   										    
	W25QXX_CS(0);                           //使能器件   
    SPI2_ReadWriteByte(W25X_FastReadData);	//发送快速读取命令（SPI CLK可以高达104Mhz）  
